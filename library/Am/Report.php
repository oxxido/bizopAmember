...
class Am_Report_Csv extends Am_Report_Output
{
    ...
    public function render()
    {
        ...
        foreach ($this->result->getLines() as $line)
        {
            $out .= $this->filterValue($line->getLabel(), self::DELIM) . self::DELIM;
        }
        $out .= "\r\n";
        ...
        {
            if (!$point->hasValues()) continue;
            $out .= $this->filterValue($point->getLabel(), self::DELIM) . self::DELIM;
            $i = 0;
            foreach ($this->result->getLines() as $line)
            {
                $out .= $this->filterValue($line->formatValue($point->getValue($line->getKey())), self::DELIM) . self::DELIM;
                @$lines[$i] = $line;
                @$totals[ $i++ ] += $point->getValue($line->getKey());
            }
            $out .= "\r\n";
        ...
        foreach ($totals as $k => $tt)
        {
            ...
            if ($tt > 0)
            {
                ...
                foreach ($totals as $v)
                    $out .= $this->filterValue($lines[$k]->formatValue($v), self::DELIM) . self::DELIM;
                $out .= "\r\n";
                break;
            }
        }
