...
class Am_Grid_Filter_Product extends Am_Grid_Filter_Abstract
{
    ...
    protected function applyFilter()
    {
        ...
        if (!@$this->vars['filter']['show_archived']) {
    ...
    public function renderShowDisabled()
    {
        return sprintf('<label>
            ...
            sprintf('<label>
                <input type="hidden" name="%s_filter[show_archived]" value="0" />
                <input type="checkbox" name="%s_filter[show_archived]" value="1" %s /> %s</label>',
            $this->grid->getId(), $this->grid->getId(),
            (@$this->vars['filter']['show_archived'] == 1 ? 'checked' : ''),
            Am_Controller::escape(___('show archived products')));
    }
...
class Am_Grid_Action_Group_Archive extends Am_Grid_Action_Group_Abstract
{
    ...
    public function __construct($archive = true)
    {
        ...
        parent::__construct(
                ...
                $archive ? ___("Archive") : ___("Un-archive")
        );
    }
...
class AdminProductsController extends Am_Controller_Grid
{
    ...
    public function createGrid()
    {
        ...
        $grid->actionGet('delete')->setIsAvailableCallback(function($rec) {return $rec->is_archived;});
    ...
    public function getTrAttribs(& $ret, $record)
    {
        if ($record->is_archived) {
            $ret['class'] = isset($ret['class']) ? $ret['class'] . ' disabled archived' : 'disabled archived';
        }
